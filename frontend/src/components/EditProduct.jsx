...
import { addProduct, fetchProductById, updateProductById } from "../apis/api";

const EditProduct = ({ setShowEditForms, editProductId }) => {
  ...
  useEffect(() => {
    const fetchProductDetails = async () => {
      try {
        const res = await fetchProductById(editProductId);
        console.log(res.data);
        const { name, description, price, category, image_urls } = res.data;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      response = await updateProductById(editProductId, formData);
      ...
      setMessage("Product Edited Succesfuy.");
        ...
        <div className="bg-white border border-gray-300 rounded-xl shadow-lg w-full max-w-md p-4 relative">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex justify-end gap-3 pt-3">
              <button
                type="button"
                onClick={() => setShowEditForms(false)}
